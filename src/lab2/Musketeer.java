package lab2;

public class Musketeer extends Piece {

    public Musketeer() {
        super("X", Type.MUSKETEER);
    }

    /**
     * Returns true if the Musketeer can move onto the given cell.
     * @param cell Cell to check if the Musketeer can move onto
     * @return True, if Musketeer can move onto given cell, false otherwise
     */
    @Override
    public boolean canMoveOnto(Cell cell) { // TODO
        
    	// Checks if the cell's piece is not null, and if it isn't then check that it has a guard on it
    	// If it fulfills both these conditions, a musketeer can move onto that space
    	if (cell.getPiece() != null && cell.getPiece().getType() == Type.GUARD) {
    		
    		return true;

		}
    	else {
			return false;
		}
    	
    	
    }
    
}
